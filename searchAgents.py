...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    visited = tuple(corner == self.startingPosition for corner in self.corner)
    self.startStart = (self.startingPosition, visited)
  ...
  def getSuccessors(self, state):
    ...
    x, y = position
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      dx, dy = Actions.derectionToVector(action)
      nextx, nexty = int(x + dx), int(y + dy)
      if not self.walls[nextx][nexty]:
...
def cornersHeuristic(state, problem):
  ...
  unvisited_corners = [corner for i, corner in enumerate(corners) if not visited[i]]

  distance = [util.manhattanDistance(currentPosition, corner) for corner in unvisited_corners]
  if distance:
    return max(distance)
...
def foodHeuristic(state, problem):
  ...
  heuristic_value = max_distance + (food_count - 1) * min_distance
